...
/// Flutter tree.
class AppGameWidget<T extends Game> extends StatefulWidget {
  ...
  /// Initial mouse cursor for this [AppGameWidget]
  ...
  @override
  _AppGameWidgetState<T> createState() => _AppGameWidgetState<T>();
}

class _AppGameWidgetState<T extends Game> extends State<AppGameWidget<T>> {
  ...
  @override
  void didUpdateWidget(AppGameWidget<T> oldWidget) {
  ...
  @override
  Widget build(BuildContext context) {
    Widget internalAppGameWidget = _GameRenderObjectWidget(widget.game);
    ...
    if (hasBasicDetectors) {
      internalAppGameWidget = applyBasicGesturesDetectors(
        widget.game,
        internalAppGameWidget,
      );
    } else if (hasAdvancedDetectors) {
      internalAppGameWidget = applyAdvancedGesturesDetectors(
        widget.game,
        internalAppGameWidget,
      );
    }
    ...
    if (hasMouseDetectors(widget.game)) {
      internalAppGameWidget = applyMouseDetectors(
        widget.game,
        internalAppGameWidget,
      );
    }

    final stackedWidgets = [internalAppGameWidget];
